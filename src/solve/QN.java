...
public class QN {
    ...
    PrintWriter utfil = new PrintWriter(new BufferedWriter(new FileWriter("PATH/TO/PRINT/RESULT/FILE")));
    ...
    for (int i = 0; i < listOfFiles.length; i++) {
        ...
        for (int temp = 0; temp < nSentence.getLength(); temp++) {
            Element eSentence = (Element) nSentence.item(temp);
            ...
            if (firstWord.equals("D") || firstWord.equals("N") || lastWord.equals("?")) {
            continue;
            }

            boolean inteAktuellt = false;
            for (int j = 0; j < nToken.getLength(); j++) {
            NodeList inteWords = ((Element) nToken.item(j)).getElementsByTagName("word");
            String inteWord = inteWords.item(0).getTextContent().toLowerCase();

            if (inteWord.equals("inte") || inteWord.equals("inget")) {
                for (int k = j; k < nToken.getLength(); k++) {
                NodeList aktuellWords = ((Element) nToken.item(k)).getElementsByTagName("word");
                String aktuellWord = aktuellWords.item(0).getTextContent().toLowerCase();
                if (aktuellWord.equals("aktuellt")) {
                    inteAktuellt = true;
                }
                }
            }
            if (inteWord.equals("år")) {
                for (int k = j; k < nToken.getLength(); k++) {
                NodeList aktuellWords = ((Element) nToken.item(k)).getElementsByTagName("word");
                String aktuellWord = aktuellWords.item(0).getTextContent().toLowerCase();
                if (aktuellWord.equals("sen") || aktuellWord.equals("sedan")) {
                    inteAktuellt = true;
                }
                }
            }
        ...
        for (int nFil = 0; nFil < afacit.size(); nFil++) {
            ...
            if (Integer.parseInt(rad[13]) != svar) {
                antalFel = antalFel + 1;
